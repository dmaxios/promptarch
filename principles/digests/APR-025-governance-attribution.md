...
> **Generated digest of [APR-025 — A Governance-Attribution Principle for Promptware](../APR-025-governance-attribution.md) v0.2.0.** The full APR is authoritative — read it for motivation, prior art, and worked detail. Do not edit by hand.
...
- **The announcement is a projection**: any in-band signal (banner/badge/statusline) MUST be **rendered by the runtime from the record — never authored by the model** (a model-authored indicator is an APR-003 §5.1 claim, and field-tested unreliable even when instructed). An announcement that can print without its record is non-conformant.
- **Reserved classes**: a class MUST NOT be emitted before its deriving predicate exists — reserve it, and name the transitional window in consumer doctrine (else the schema silently contradicts the contract).
- **Never gates**: attribution records and announces; it MUST NOT block a turn.
- **The padlock caveat is normative**: consumer docs MUST state that absence of an ungoverned-warning is not proof of governance for turns the promptware never saw — and absence of an advisory-class record is not evidence no advisory component ran on recorder-less channels.
...
The sub-promptware case (promptware never loaded) is unreportable from inside. Registered as **R15**: a conforming harness SHOULD stamp every consumer-visible turn with turn-provenance — strongest form: **unconditional stamping with supersession** (harness writes a baseline record per turn; the promptware's record supersedes it), making absence-as-evidence two-sided. Until then: positive attribution reliable, complete accounting honestly partial.
...
---
*Source: [APR-025 — A Governance-Attribution Principle for Promptware](../APR-025-governance-attribution.md) v0.2.0 · regenerate this digest whenever the source changes.*
